...
export const CartDrawer: React.FC = () => {
  ...
  return (
    <>
            ...
            <div className="p-6 border-b border-surface-border flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <span className="text-[11px] font-mono text-text-secondary">
                  ({items.reduce((s, i) => s + i.quantity, 0)} HEIRLOOMS)
                </span>
              </div>
              ...
              {items.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center space-y-4 py-16">
                  ...
                  <p className="font-serif text-lg text-text-secondary">
                    Your trousseau bag is currently empty.
                  </p>
                  <p className="text-xs text-text-tertiary max-w-xs leading-relaxed font-light">
                    Explore our master-loom collections to inspect pure mulberry silks, authentic Korvai interlocks, and antique matte zari.
                  </p>
                  <button
                    onClick={closeCart}
                  ...
                  >
                    ...
                    <div className="flex-1 flex flex-col justify-between">
                      <div>
                        ...
                        <div className="text-[11px] font-mono text-accent-zari-hover">
                        ...
                        <div className="mt-2 space-y-0.5 text-[10px] font-mono text-text-tertiary">
                          <div>
                            • Fall & Pico: {item.customizations?.fallPico === "silk-rolled" ? "Silk-Rolled (+15)" : item.customizations?.fallPico === "unaltered" ? "Unaltered" : "Hand-Stitched (Free)"}
                          </div>
                          {item.withBlouse && (
                            <div className="text-accent-zari-hover">
                              • Blouse: {item.customizations?.blouse.styleOption === "custom-tailored" ? `Custom Tailored (+${formatPrice(item.customizations.blouse.priceUSD)})` : `Unstitched (+${formatPrice(item.product.blouseOption.priceUSD)})`}
                            </div>
                          )}
                          {item.customizations?.petticoat?.enabled && (
                            <div>
                              • Mermaid Shaper (Size {item.customizations.petticoat.waistSize}): +{formatPrice(35)}
                            </div>
                          )}
                          {item.customizations?.prePleated?.enabled && (
                            <div>
                              • Pre-Pleated Drape ({item.customizations.prePleated.waistInches} in.): +{formatPrice(35)}
                            </div>
                          )}
                        </div>
                      </div>
            ...
            {items.length > 0 && (
              ...
              >
                ...
                <div className="flex items-center justify-center gap-2 text-[10px] font-mono text-text-tertiary">
                  ...
                  <span>SILK MARK CERTIFIED · INSURED DHL TRANSIT</span>
                </div>
              </div>
            )}
          </aside>
      ...
      {/* Complete Checkout Experience Modal */}
