  ...
  return (
    <>
      {/*
        ...
        Both the announcement banner AND the nav header are fixed together
        so the nav never slides over the banner text on scroll.
        The page body is pushed down via a spacer div with matching height.
      */}
      <div
      ...
      >
        ...
        <div
          ref={bannerRef}
          className="bg-canvas-elevated border-b border-surface-border/60 py-1.5 px-4 text-center text-[9px] sm:text-[10px] font-mono tracking-[0.18em] text-text-secondary uppercase"
        >
          <span className="hidden md:inline">
            Complimentary Hand-Stitched Fall &amp; Pico · Silk Mark Certified Heirlooms · Worldwide DHL Express DDP · 7-Day In-Home Inspection
          </span>
        ...
        <header
          className={`transition-all duration-500 ${
            scrolled
              ? "bg-canvas-base/92 backdrop-blur-md border-b border-surface-border py-2 sm:py-2.5 shadow-xs"
              : "bg-canvas-base/70 backdrop-blur-sm border-b border-surface-border/30 py-3 sm:py-5"
          }`}
        >
      ...
      {/* Page body spacer — pushes content below the fixed header block */}
      <div
        className="pointer-events-none"
        aria-hidden="true"
        style={{ height: "calc(env(safe-area-inset-top, 0px) + 28px + var(--header-nav-height, 64px))" }}
      />
